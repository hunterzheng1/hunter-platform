...
export class MemoryBranchSnapshotPort implements BranchSnapshotRepositoryPort, BlobReadPort, CursorVerifierPort, RestoreConflictReadPort {
  ...
  async getSnapshotByVersionRef(input: { actor_id: string; allowed_project_ids: readonly string[]; project_id: string; branch_name: string; project_version: string }): Promise<{ actor_id: string; identity: SnapshotIdentity; record: BranchSnapshotRecord } | null> { const record = this.records.find((v) => v.project_id === input.project_id && v.branch_name === input.branch_name && v.project_version === input.project_version); if (record === undefined) return null; const identity: SnapshotIdentity = { project_id: record.project_id, branch_name: record.branch_name, commit_sha: record.commit_sha, project_version: record.project_version, artifact_id: record.artifact_id, manifest_hash: record.manifest_hash }; return structuredClone({ actor_id: input.actor_id, identity, record }); }
  async getSnapshotPredecessor(input: { actor_id: string; allowed_project_ids: readonly string[]; identity: SnapshotIdentity }): Promise<{ actor_id: string; identity: SnapshotIdentity; record: BranchSnapshotRecord } | null> { const target = this.records.find((v) => identityKey(v) === identityKey(input.identity)); if (target === undefined) return null; const branchRecords = this.records.filter((v) => v.project_id === target.project_id && v.branch_name === target.branch_name).sort(compareRecords); const index = branchRecords.findIndex((v) => identityKey(v) === identityKey(target)); const predecessor = index < 0 ? undefined : branchRecords[index + 1]; if (predecessor === undefined) return null; const identity: SnapshotIdentity = { project_id: predecessor.project_id, branch_name: predecessor.branch_name, commit_sha: predecessor.commit_sha, project_version: predecessor.project_version, artifact_id: predecessor.artifact_id, manifest_hash: predecessor.manifest_hash }; return structuredClone({ actor_id: input.actor_id, identity, record: predecessor }); }
