...
import { canonicalJson, remoteSyncLeaseSchema } from "@hunter-harness/contracts";
...
import type { PgRemoteSyncCommitOptions, PgRemoteSyncCommitPort, PgRemoteSyncCommitResult } from "./ports.js";
...
function validateInput(input: Parameters<BranchSnapshotCommitPort["commitSnapshot"]>[0]): void {
  ...
  if (input.lease_fence !== undefined) {
    const parsed = remoteSyncLeaseSchema.safeParse(input.lease_fence);
    if (!parsed.success || parsed.data.project_id !== source.project_id ||
        parsed.data.branch_name !== source.branch_name || parsed.data.actor_id !== source.actor_id) {
      throw error("BRANCH_SNAPSHOT_COMMIT_INPUT_INVALID");
    }
  }
}
...
function payloadHash(input: Parameters<BranchSnapshotCommitPort["commitSnapshot"]>[0]): string {
  return hash({
    ...
    ...(input.lease_fence === undefined ? {} : { lease_fence: input.lease_fence }),
    source: input.source,
    seed: input.seed,
  });
}
...
function stableConflict(reason: "BRANCH_SNAPSHOT_IDENTITY_CONFLICT" | "BRANCH_SNAPSHOT_REVISION_CONFLICT" | "BRANCH_SNAPSHOT_LEASE_FENCED"):
BranchSnapshotDurableCommitResult {
...
interface ActiveLeaseFenceRow extends QueryResultRow {
  lease_id: string;
  lease_token: string;
  generation: string | number;
...
function conflictFromCause(value: unknown): BranchSnapshotDurableCommitResult | null {
  ...
  if (value instanceof Error && value.message === "BRANCH_SNAPSHOT_LEASE_FENCED") {
    return stableConflict("BRANCH_SNAPSHOT_LEASE_FENCED");
  }
  return null;
}
...
export function createPgRemoteSyncCommitPort(options: PgRemoteSyncCommitOptions): PgRemoteSyncCommitPort {
  ...
  type CommitInput = Parameters<BranchSnapshotCommitPort["commitSnapshot"]>[0];

  async function commitSnapshotWithin(client: PoolClient, rawInput: CommitInput): Promise<PgRemoteSyncCommitResult> {
    ...
    if (input.lease_fence !== undefined) {
      const active = await client.query<ActiveLeaseFenceRow>(
      ...
      const fence = active.rows[0];
      if (fence === undefined || fence.lease_id !== input.lease_fence.lease_id ||
          fence.lease_token !== input.lease_fence.lease_token ||
          generationNumber(fence.generation) !== input.lease_fence.generation ||
          canonicalUploadedAt(fence.expires_at instanceof Date ? fence.expires_at.toISOString() : String(fence.expires_at)) !==
            input.lease_fence.expires_at ||
          !sameCanonical(fence.source_json, input.source) ||
          Date.parse(input.lease_fence.expires_at) <= Date.parse(created_at)) {
        return stableConflict("BRANCH_SNAPSHOT_LEASE_FENCED");
      }
    }
    ...
    if (JSON.stringify(storedRecord) !== JSON.stringify(storageExpected)) throw error("BRANCH_SNAPSHOT_RECORD_INVALID");
  ...
  return {
    async commitSnapshot(rawInput) {
      try {
        return await transaction(options.pool, (client) => commitSnapshotWithin(client, rawInput));
      } catch (cause) {
    ...
    async commitSnapshotWithClient(client, rawInput) {
      return commitSnapshotWithin(client, rawInput);
    },
  };
}
