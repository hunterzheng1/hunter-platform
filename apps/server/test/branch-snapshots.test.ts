...
describe("BranchSnapshotModule", () => {
  ...
  it("emits a single base64url cursor accepted by the public query and page contracts", async () => {
    ...
    const first = await module.listBranches({ ...base, cursor: null, limit: 1 });
    const cursor = first.next_cursor; const item = first.items[0];
    expect(cursor).toEqual(expect.stringMatching(/^[A-Za-z0-9_-]{16,512}$/u));
    expect(platformInformationQuerySchema.safeParse({ schema_version: 1, contract_kind: "query", view: "branch_files", project_id: base.project_id, query_scope: { actor_id: base.actor_id, accessible_project_ids: base.accessible_project_ids, content_types: ["branch_file"] }, limit: 1, cursor, cursor_verification: "server_port_required", sort: "uploaded_at_desc_snapshot_version_asc" }).success).toBe(true);
    expect(platformInformationPageSchema.safeParse({ schema_version: 1, contract_kind: "page", view: "branch_files", project_id: base.project_id, page_state: "ready", sort: "uploaded_at_desc_snapshot_version_asc", items: item === undefined ? [] : [{ item_kind: "branch_snapshot", branch_name: item.branch_name, snapshot_version: item.project_version, commit_sha: item.commit_sha, uploaded_at: item.uploaded_at, file_count: item.file_count, changed_file_count: item.changed_file_count, detail_id: `bf_${item.branch_name}~${item.project_version}`, sort_key: `${item.uploaded_at}\0${item.project_version}` }], next_cursor: cursor, failures: [] }).success).toBe(true);
    await expect(port.verify(`${Buffer.from(JSON.stringify({ actor_id: base.actor_id, project_id: base.project_id, query_kind: "branches", offset: 1 })).toString("base64url")}.legacy`, { actor_id: base.actor_id, project_id: base.project_id, query_kind: "branches" })).rejects.toThrow("BRANCH_SNAPSHOT_CURSOR_INVALID");
  });
