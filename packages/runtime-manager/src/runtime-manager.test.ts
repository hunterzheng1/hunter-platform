...
describe("RuntimeManager", () => {
  it("persists assignment authority and survives manager restart", async () => {
    ...
    const first = new RuntimeManager(database, journal).requestAssignment(input);
    expect(new RuntimeManager(database, journal).requestAssignment(input)).toEqual(first);
  ...
  it.each(["deny", "require_approval"] as const)("creates no launch operation for %s", async (policyDecision) => {
    const { database, journal, operation, leaseIds } = await harness();
    expect(() => new RuntimeManager(database, journal).requestAssignment({ commandId: `assign:${policyDecision}`, expectedVersion: 0, operation, policyDecision, capabilityReceipt: capability(), requiredLeaseIds: leaseIds, now: new Date(now) })).toThrow(/POLICY_NOT_ALLOWED/u);
  ...
  it("fails closed for missing capability or durable Lease receipts", async () => {
    ...
    expect(() => new RuntimeManager(database, journal).requestAssignment({ commandId: "assign:no-cap", expectedVersion: 0, operation, policyDecision: "allow", capabilityReceipt: unsupported, requiredLeaseIds: [], now: new Date(now) })).toThrow(/CAPABILITY_NOT_PROVEN/u);
    expect(() => new RuntimeManager(database, journal).requestAssignment({ commandId: "assign:no-lease", expectedVersion: 0, operation, policyDecision: "allow", capabilityReceipt: receipt, requiredLeaseIds: [], now: new Date(now) })).toThrow(/LEASE_RECEIPT_REQUIRED/u);
    database.close();
  });
});
