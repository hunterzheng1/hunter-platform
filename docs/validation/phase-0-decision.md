...
> 冻结快照边界：从“决策摘要”到“证据自审”保留 Outcome 5 冻结时的历史事实与证据判定；
> 它不是当前工具清单。后续 Orca/AO 本机状态只以文末按日期追加的“后续证据”为准，
> 这些追加证据没有改变 Outcome 5 或 Provider 采用状态。2026-07-23 修订只澄清 First Vertical Slice
> 的门禁范围，不追溯改写任何 `PASS`、`FAIL`、`BLOCKED` 或 `NOT_PROVEN`。
...
Hunter Foundation 已通过 provider-neutral contracts、确定性 Fake Runtime、崩溃恢复和 Windows/Ubuntu CI。这些结果只证明 Hunter 自己的契约与失败语义，不证明任何真实 Provider。Outcome 5 允许继续 Foundation 维护、Fake contract 验证，以及确定性 Fake 驱动的 First Vertical Slice 开发和自动验收，包括在本地或 CI 运行 `pack:win` 并生成未签名的 Windows NSIS 测试产物。Fake-only 路径只证明 Hunter 契约与产品链路；NSIS 测试产物只增加打包验证，不证明任何真实 Provider。Phase 0 Gate A、作为真实 Provider 验收的 fallback typed scenario、First Vertical Slice 的真实 Provider 集成与验收及真实 Provider 能力宣传均保持阻断；生产签名、分发和发布继续阻断，直到计划要求的证据齐备。
...
- Foundation 维护、Fake contract 验证，以及确定性 Fake 驱动的 First Vertical Slice 开发和自动验收可以继续。
- Fake-only First Vertical Slice 只证明 Hunter 契约与产品链路，不证明或采用任何真实 Provider，也不解除 Phase 0 Gate A。
- 允许在本地或 CI 运行 `pack:win`，生成未签名、仅用于自动验收的 Windows NSIS 测试产物。
- 该 NSIS 测试产物只证明 Fake-only 产品链路与打包验证，不是任何真实 Provider 的原子能力收据；不得签名、分发或作为生产发布。
- Phase 0 Gate A、First Vertical Slice 的真实 Provider/Connector 集成与验收及真实 Provider 能力宣传继续阻断。
- 生产签名、分发和发布继续阻断。
...
以下四项是 2026-07-21 冻结 Outcome 5 时的审计快照，不用于描述后续追加的 Orca/AO 本机验证：

- 冻结时没有真实 Provider PASS；Hunter contract-only 结果明确与采用判定分离。
- 冻结时 Doctor envelope 已应用 schema v1 脱敏；扫描未发现 token、cookie、API key、完整环境变量、私有 Prompt 或绝对用户隐私路径。
- 冻结时没有执行真实 Provider 写操作，因此没有 Provider fixture 清理收据；Foundation 中会写仓库的测试只使用自动创建的临时 Git fixture。
- 冻结时没有安装、登录、付费、权限扩大、远端 Provider 写入或权限绕过命令。

## P0-RUNTIME-01 后续证据（2026-07-22）

用户完成 Orca 安装和本地 CLI 注册后，Doctor 重新执行并产生新证据：Orca 的 executable availability 与 `status --json` 非交互状态可用性为 `DETECTED`，精确版本仍为 `null`；原子 `discover_runtime` receipt 为 `PASS`。详见 [`orca-windows-provider.md`](orca-windows-provider.md) 与 [`evidence/orca/preflight.json`](evidence/orca/preflight.json)。

该证据没有改变 Outcome 5。当前安装版本的公开 `worktree create` 命令没有在创建前固定 checkout 到 Hunter 临时根目录的目标路径参数，公开 `repo` 命令也没有形成完整 deregistration cleanup receipt。为遵守“所有写仓库探针只运行在自动创建的临时 Git fixture”约束，本轮没有执行 repo/worktree/terminal 写操作，也没有关闭或重启 Orca。

因此 workspace create/find、terminal launch/observe/interrupt、restart/reconcile、workspace/session identity、安全默认值和移动配对继续为 `NOT_PROVEN`；Phase 0 Gate A、First Vertical Slice 的真实 Provider 集成与验收及真实 Provider 能力宣传继续阻断；生产签名、分发和发布继续阻断。该证据不阻断 Fake-only First Vertical Slice 的开发、自动验收或未签名 Windows NSIS 测试打包；不选择或 Fork Orca，不计算 Orca Capability 等级。

## P0-RUNTIME-01 Agent Orchestrator fallback 后续证据（2026-07-22）

用户授权安装官方 Agent Orchestrator `v0.10.3` 后，Hunter 在隔离 daemon data/run file/port
和自动临时无 remote Git fixture 中执行了 Task 9。详见
[`agent-orchestrator-fallback.md`](agent-orchestrator-fallback.md) 与
[`evidence/agent-orchestrator/fallback.json`](evidence/agent-orchestrator/fallback.json)。

本机收据证明 runtime discovery、project registration、project cleanup 与受支持 CLI JSON
读取为 `PASS`。同时，安装包内 CLI 报告 `ao version dev`，固定版本为 `FAIL`；agent catalog
初次人工 refresh 在 10 秒时间盒内超时，最终 typed scenario 虽成功刷新，但未证明场景实际配置的
`codex` worker/orchestrator 同时 installed 与 authorized，因此 agent readiness 为 `NOT_PROVEN`；
catalog 原始身份清单未写入 evidence。workspace create/find、process/terminal launch 与 observe
因配置 Agent readiness 未证明而为 `BLOCKED`；
公开 CLI 没有 raw terminal interrupt，因此 interrupt 为 `FAIL`。没有 native session identity，
因此 restart/reconcile 与 workspace/session identity 仍为 `NOT_PROVEN`。

场景没有调用 `spawn`、没有启动或计费真实 agent、没有 remote、没有读取私有 SQLite/API/WS，
也没有使用 `--skip-agent-check`、`--yes` 或权限绕过。删除前 session list 观察为 0；cleanup
收据只证明目标 project 按精确 id 删除且后续 lookup 返回 `project not found`，不声称最终全局
project/session 计数。临时 Git fixture 已删除。

这些新证据不会把 AO 选为 Provider，也不会改变 Outcome 5。Phase 0 Gate A、First Vertical
Slice 的真实 Provider/Connector 集成与验收及真实 Provider 能力宣传继续阻断；生产签名、分发和发布继续阻断。
Fake-only First Vertical Slice 的开发、自动验收和未签名 Windows NSIS 测试打包不受阻断，Capability 等级仍不得计算或宣传。
...
Gate A、First Vertical Slice 的真实 Provider 集成与验收及真实 Provider 能力宣传阻断保持不变；
生产签名、分发和发布继续阻断。该证据不阻断 Fake-only First Vertical Slice 的开发、自动验收或未签名 Windows NSIS 测试打包，不按产品名称计算 L0-L3。
