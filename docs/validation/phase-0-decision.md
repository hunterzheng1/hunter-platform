...
Hunter Foundation 已通过 provider-neutral contracts、确定性 Fake Runtime、崩溃恢复和 Windows/Ubuntu CI。这些结果只证明 Hunter 自己的契约与失败语义，不证明任何真实 Provider。Outcome 5 允许继续 Foundation 维护、Fake contract 验证，以及确定性 Fake 驱动的 First Vertical Slice 开发和自动验收；Fake-only 路径只证明 Hunter 契约与产品链路，证据范围始终为 `contract_only`。Phase 0 Gate A、作为真实 Provider 验收的 fallback typed scenario、First Vertical Slice 的真实 Provider 集成与验收、真实 Provider 能力宣传及生产发布均保持阻断，直到计划要求的证据齐备。
...
- Phase 0 Gate A、First Vertical Slice 的真实 Provider/Connector 集成与验收、真实 Provider 能力宣传、生产打包和发布继续阻断。
...
因此 workspace create/find、terminal launch/observe/interrupt、restart/reconcile、workspace/session identity、安全默认值和移动配对继续为 `NOT_PROVEN`；Phase 0 Gate A、First Vertical Slice 的真实 Provider 集成与验收、真实 Provider 能力宣传及生产发布继续阻断。该证据不阻断 Fake-only First Vertical Slice 的开发和自动验收；不选择或 Fork Orca，不计算 Orca Capability 等级。
...
Slice 的真实 Provider/Connector 集成与验收、真实 Provider 能力宣传及生产发布继续阻断；
Fake-only First Vertical Slice 的开发和自动验收不受阻断，Capability 等级仍不得计算或宣传。
...
Gate A、First Vertical Slice 的真实 Provider 集成与验收、真实 Provider 能力宣传及生产发布阻断保持不变；
该证据不阻断 Fake-only First Vertical Slice 的开发和自动验收，不按产品名称计算 L0-L3。
