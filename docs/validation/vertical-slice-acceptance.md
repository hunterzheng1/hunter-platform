...
提交前新增的 `Vertical slice / windows-latest` 与
`Vertical slice / ubuntu-latest` 尚未在远端运行，当前如实记录为 `PENDING`。
它们将安装 Chromium、运行 lint/typecheck/unit/browser，并仅在 Windows 运行
未签名 `pack:win`。已有 Task 19 quality CI 不替代这两项新门禁。
